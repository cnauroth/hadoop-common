...
public class AzureRingBuffer<E> implements Queue<E>
{
	...
	/**
	 * Default constructor creating a ring buffer with a default capacity.  The ring buffer does
	 * not auto-grow.
	 */
    ...
    /**
	 * Creates a ring buffer with a non-default capacity.  The ring buffer does not auto-grow.
	 *
	...
	/**
	 * Creates a ring buffer with a non-default capacity. The ring buffer is allowed to
	 * auto-grow.
	 *
	 * @param aCapacity     
	 *			Non-default capacity of the ring buffer.
	 * @param canAutoGrow 
	 *			Set to true if the ring buffer is allowed to grow, otherwise it is false.
	 */
	 ...
	 *
	 * @param anElement
	 *			Add this data element to the ring buffer.
	 * @return boolean
	 *			True if the element was successfully added to the ring buffer.
	 */
	@Override
	 ...
	 *
	 * @param aCollection
	 *			A collection of data elements to add to the ring buffer.
	 * @return boolean
	 *			True if the entire collection is added to the ring buffer.
	 */
	@Override
	public boolean addAll(Collection<? extends E> aCollection)
	{
		...
		// Successfully added elements to the ring buffer.  Adding empty collections are
	 ...
	 *
	 * @param anObject
	 *			Check for the existence of this object in the ring buffer.
	 * @return boolean
	 *			True if the element is found in the ring buffer, and false otherwise.
	 */
	 @Override
	 public boolean contains(Object anObject)
	 {
	 	...
	 	while (iter.hasNext ())
	 	{
	 		...
	 		if (null == anObject ? null == currentElement 
	 							 : anObject.equals (currentElement))
	 		{
	 			return true;
	 		}
	 	}
	  ...
	  * @return boolean
	  *			True if all objects in the collection are in the ring buffer.
	  */
	 @Override
	 ...
	 * 
	 * @return E
	 *			Current head element of the ring buffer.
	 */
	@Override
	public E element()
	{
		// Return the current head element if the ring buffer contains any elements at all.
		//
		if (0 < m_size)
		{
		...
		// The ring buffer is empty, throw an exception indicating no such element exists.
		//
	 ...
	 *
	 * @return boolean
	 *			True if the ring buffer auto-grows on reaching its capacity.
	 */
	 ...
	 *
	 * @return boolean
	 *			True if the ring buffer contains no elements.
	 */
	@Override
	 ...
	 *
	 * @return Iterator<E>
	 *			Iterator over elements in the ring buffer.
	 */
	@Override
	 ...
	 *
	 * @param anElement
	 *			The element to be inserted into the ring buffer.
	 * @return boolean
	 *			True when the element is successfully inserted into the ring buffer.
	 */
	@SuppressWarnings("unchecked")
	@Override
	public boolean offer (E anElement)
	{
		if (m_data.length == m_size)
		{
			...
			if (m_autoGrow)
			{
				// Grow the ring buffer by reallocating with double the current ring buffer
				// capacity.
				//
	...
	public E poll()
	{
		...
		if (null != headElement)
		{
			// Assertion: The ring buffer is not empty so its size should be greater than 0.
			//
			...
			{
				// Wrap the head since the head index is at the capacity of the ring buffer.
				//
				m_head = 0;
			}
		}
	 ...
	 *
	 * @return boolean
	 *			True if object is successfully removed from the ring buffer.
	 */
	@Override
	...
	/**
	 * Dynamically set whether or not the ring buffer automatically grows when it reaches 
	 * capacity.
	 *
	 * @param canAutoGrow
	 *			Has a value of true for auto-growth and a value of false for fixed capacity.
	 */
	 ...
	 */
	 ...
	 /**
	  * Returns an array containing all of the elements in this ring buffer. Since the ring buffer
	  ...
	  *
	  * @return Object []
	  *				Array of all elements in the ring buffer.
	  */
	 @Override
	...
	/**
	 * Returns an array containing all of the elements in this ring buffer; the runtime type of
	 * the array returned is that of the specified array. If the ring buffer fits in the specified
	...
	public <T> T[] toArray (T[] anArray)
	{
		if (anArray.length < m_size)
		{
			...
			// ring buffer. Reflect on array type to create a new array type that is the
			// correct size.
			//
			anArray = (T[]) Array.newInstance(
