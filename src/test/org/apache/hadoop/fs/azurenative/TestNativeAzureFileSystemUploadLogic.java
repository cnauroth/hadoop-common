...
public class TestNativeAzureFileSystemUploadLogic {
  ...
  private void testConsistencyAfterManyFlushes(FlushFrequencyVariation variation)
      throws Exception {
    // Use small buffer sizes so I can exercise more of the code
    // without having to process huge files.
    testAccount.getFileSystem().getConf().setInt(
        NativeAzureFileSystem.AZURE_OUTPUT_STREAM_BUFFER_SIZE_PROPERTY_NAME,
        500);
    testAccount.getFileSystem().getConf().setInt(
        NativeAzureFileSystem.AZURE_RINGBUFFER_CAPACITY_PROPERTY_NAME,
        3);
