...
import static org.apache.hadoop.fs.azurenative.AzureNativeFileSystemStore.*;
import org.junit.*;
...
public class TestBlobDataValidation {
  ...
  /**
   * Test that we get blob-level MD5 storage and validation if we
   * specify that in the configuration.
   */
  @Test
  ...
  private void testStoreBlobMd5(boolean expectMd5Stored) throws Exception {
    ...
    OutputStream outStream =
        testAccount.getFileSystem().create(testFilePath);
    ...
    blob.commitBlockList(Arrays.asList(new BlockEntry[]
        {
          new BlockEntry(newBlockId, BlockSearchMode.UNCOMMITTED)
        }));
  ...
  /**
   * Test that we don't check block-level MD5 if we
   * specify that in the configuration.
   */
  @Test
  ...
  /**
   * Connection inspector to check that MD5 fields for content is
   * set/not set as expected.
   */
  private static class ContentMD5Checker
    ...
    public void eventOccurred(ResponseReceivedEvent eventArg) {
      ...
      if (isGetRange(connection)) {
        checkObtainedMd5(connection.getHeaderField(
            Constants.HeaderConstants.CONTENT_MD5));
      } else if (isPutBlock(connection)) {
        checkObtainedMd5(connection.getRequestProperty(
            Constants.HeaderConstants.CONTENT_MD5));
      }
    }
    ...
    private static boolean isGetRange(HttpURLConnection connection) {
      return connection.getRequestMethod().equals("GET") &&
          connection.getHeaderField(
              Constants.HeaderConstants.STORAGE_RANGE_HEADER) != null;
    }
  }
